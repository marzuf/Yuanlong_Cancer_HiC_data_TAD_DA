...
# Rscript check_dopar.R ENCSR079VIJ_G401_40kb TCGAkich_norm_kich
...
#   gene2 <- gene_pair[2]
#   
#   stopifnot(gene1 %in% rownames(qqnormDT))
#   stopifnot(gene2 %in% rownames(qqnormDT))
#   
#   as.numeric(get_ang_dist(qqnormDT[paste0(gene1),],
#                           qqnormDT[paste0(gene2),]))
#   
# })
# 
# 
...
stopifnot(nrow(all_angDist) == ncol(all_genes_cmbs))

outFile <- file.path(outFolder, "all_angDist_DT.Rdata" )
...
#   gene1 <- gene_pair[1]
#   gene2 <- gene_pair[2]
#   stopifnot(gene1 %in% rownames(qqnormDT_cond1))
#   stopifnot(gene2 %in% rownames(qqnormDT_cond1))
#   as.numeric(get_ang_dist(qqnormDT_cond1[paste0(gene1),],
#                qqnormDT_cond1[paste0(gene2),]))
# })
...
stopifnot(nrow(all_angDist_cond1) == ncol(all_genes_cmbs))

outFile <- file.path(outFolder, "all_angDist_DT_cond1.Rdata" )
...
#   gene2 <- gene_pair[2]
#   
#   stopifnot(gene1 %in% rownames(qqnormDT_cond2))
#   stopifnot(gene2 %in% rownames(qqnormDT_cond2))
#   
#   as.numeric(get_ang_dist(qqnormDT_cond2[paste0(gene1),],
#                           qqnormDT_cond2[paste0(gene2),]))
#   
# })
# 
...
stopifnot(nrow(all_angDist_cond2) == ncol(all_genes_cmbs))



outFile <- file.path(outFolder, "all_angDist_DT_cond2.Rdata" )
